...
def __Stock_Order(Target_Expcode, Order_type, Price) :
    ...
    File_Name = "D:\\Python\\Log\\Order_History.txt"
    output = open(File_Name, "a")
    output.write("매수시도 : %s ---> 종목 : %s, 수량 : %d, 가격 : %s, 1회제한금액 : %d\n" % (datetime.datetime.today().strftime("%Y%m%d %H:%M:%S"), Target_Expcode, Buy_Qty, Price, OneTime_Price))
#     output.write(Buy_Qty, Price, OneTime_Price)
#     output.write("매수시도 ---> : 매수수량 : %d, 가격 : %d 허용가격 : %d \n" %  Buy_Qty, Price, OneTime_Price))
    output.close() # ---> close 가 되어야 write 처리가 완료된다
#     (datetime.datetime.today().strftime("%Y%m%d %H%:M:%S"),
...
#         print("주문번호 : %s, 종목번호 : %s, 구분 : %s, 주문수량 : %s, 주문가격 : %s, 체결수량 : %s, 체결가격 : %s, 미체결잔량 : %s, 확인수량 : %s, 상태 : %s" % (ordno, expcode, medosu, qty, price, cheqty, cheprice, ordrem, cfmqty, status))
        ...
        output = open(File_Name, "a")
        output.write("%s : " % (datetime.datetime.today().strftime("%Y%m%d %H:%M:%S")))
        output.write("주문번호 : %s, 종목번호 : %s, 구분 : %s, 주문수량 : %s, 주문가격 : %s, 체결수량 : %s, 체결가격 : %s, 미체결잔량 : %s, 확인수량 : %s, 상태 : %s" % (ordno, expcode, medosu, qty, price, cheqty, cheprice, ordrem, cfmqty, status))
        output.write("\n")
        output.close() # ---> close 가 되어야 write 처리가 완료된다
...
#         print("테스트하자")
        CallNothing = 0
        return CallNothing
#     elif ('매수' in order_group_list) and ('체결' in order_status_list) :
    else :
        return None
...
#         if datetime.datetime.today().strftime("%M") == Term_Min : # 분단위로 restart하므로 초반에 걸리면 조건에 맞는 분이 끝날 때가지 계속 restart됨
#             print("# Restart!!!")
#             File_Name = "D:\\Python\\Log\\Login_History.txt"
#             output = open(File_Name, "a")
#             output.write("RESTART !!! ---> %s \n" % (datetime.datetime.today().strftime("%Y%m%d %H:%M:%S")))
#             output.close()
#             restart()
#             time.sleep(5)
...
#         print(Account_Expcode) # 계좌에 있는 종목 리스트
        ...
        if Target_Expcode not in Account_Expcode :
            ...
            File_Name = "D:\\Python\\Log\\Order_History.txt"
            output = open(File_Name, "a")
            output.write("자산 내 대상 종목 없음 ---> %s : 종목 %s \n" % (datetime.datetime.today().strftime("%Y%m%d %H%:M:%S"), Target_Expcode))
            output.write("\n")
            output.close() # ---> close 가 되어야 write 처리가 완료된다
